...
`internal/storage` 提供状态机抽象和具体存储实现，负责承接“Raft 已提交日志最终如何落到本地状态”。
...
- `Get` / `Scan`
  读取业务数据，并在扫描时跳过 `__meta__/` 内部键
- `Snapshot`
  把业务 KV 和成员地址一起编码导出
- `Restore`
  清空现有业务数据和成员地址后，按快照内容重建
